...
/*   Updated: 2023/07/27 15:31:48 by marvin           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	routine(t_philo *philo)
{
	...
	while (1)
	{
		...
		if (rw_val(philo->read, *philo->eat_count, 0) == rw_val(phlo->read,
...
void	*reaper(void *ptr)
{
	...
	while (1)
	{
		if (rw_val(philo->read, philo->last_eat, 0) >= rw_val(philo->read, philo->rules->death_t, 0))
		{
			ft_print(DIE, philo);
